...
class BlendedLatnetDiffusion:
    ...
    def edit_image(
        ...
        height=512,
        width=512,
        num_inference_steps=50,  # was 50 initially
        guidance_scale=7.5,
        generator=torch.manual_seed(42),
        blending_percentage=0.25,  # was .25 initially
    ):
        ...
        soft_mask, latent_mask, org_mask = self._read_mask(mask)
        ...
        t_steps = self.scheduler.timesteps[
            int(len(self.scheduler.timesteps) * blending_percentage) :
        ]
        T = len(t_steps)

        for step_i, t in enumerate(t_steps):
            if T <= 1:
                raise Exception("T value less than 2!")
            s = step_i / (T - 1)
            g_t = s**self.gamma
            m_t = torch.round(
                g_t * soft_mask
            )  # removing softness at this step to ensure text prompt is followed
            ...
            latents = latents * m_t + noise_source_latents * (1 - m_t)
    ...
    def _read_mask(self, mask, dest_size=(64, 64)):
        """
        Returns:
            soft_mask: (1,1,H,W) smoothed mask in [0, 1]
            binary_mask: (1,1,H,W) original hard mask {0, 1}
            org_mask: high-resolution original mask
        """
        org_mask = mask
        ...
        dist = ndi.distance_transform_edt(1 - m)  # distance to foreground boundary

        # soft_mask = sigmoid((R - dist)/tau)
        soft = 1.0 / (1.0 + np.exp(dist - self.R / self.tau))

        soft_mask = (
            torch.from_numpy(soft).unsqueeze(0).unsqueeze(0).half().to(self.device)
        )

        binary_mask = (
            torch.from_numpy(m).unsqueeze(0).unsqueeze(0).half().to(self.device)
        )

        return soft_mask, binary_mask, org_mask


image_save_paths = {"blended-latent-diffusion": "blended-latent-diffusion"}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--edit_method_list", nargs="+", type=str, default=["blended-latent-diffusion"]
    ...
    # eta = 0.5
    # gamma = 0.22  # gamma = log(1/2) / log(eta)
    # tau = 0.8
    # R = 4

    # bld = BlendedLatnetDiffusion(gamma=gamma, tau=tau, R=R, eta=eta)
    ...
    for key, item in tqdm(editing_instruction.items(), desc="Processing images"):
        ...
        for edit_method in edit_method_list:
            present_image_save_path = image_path.replace(
                data_path, os.path.join(output_path, image_save_paths[edit_method])
            )
